...
abstract class Util
{
  ...
  /**
   * Converts a response from the Safepay API to the corresponding PHP object.
   *
   * @param string $resource to create
   * @param array $resp the response from the Safepay API
   * @param array $opts
   *
   * @return array|SafepayObject
   */
  public static function convertToSafepayObject($resource, $resp, $opts)
  {
    ...
    if (\is_array($resp)) {
      if (isset($resource) && isset($types[$resource])) {
        ...
        $class = \Safepay\SafepayObject::class;
      }
    ...
    if (null === self::$isMbstringAvailable) {
      ...
      if (!self::$isMbstringAvailable) {
        \trigger_error('It looks like the mbstring extension is not enabled. ' .
          ...
          'support@getsafepay.com if you have any questions.', \E_USER_WARNING);
      }
    }
  ...
  {
    if ($h instanceof \Safepay\ApiResource) {
      return $h->id;
    }
    if (static::isList($h)) {
      $results = [];
